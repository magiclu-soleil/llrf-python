...
class llrf_graph_window(QtWidgets.QMainWindow):
    ...
    def calc_phase_angle(self, ch):
        ...
        try:
            ...
            #print(mag_amp)
            phase_angle = phase_set.IQ_phase_shift(ph)
            print(addr)
...
#        self.button_phase.hide()
    ...
    def first_plot(self):
        ...
        color_line =['g','b','r','y','k',(180,165,0),(128,0,128),'r', 'b']
        #**********************************************#
        ...
        if self.bram7.isChecked():
            n = min(len(self.buf_q[7]), len(self.buf_i[7]))
            self.plt_scatter[5] = self.plt_ch[1].plot(self.buf_i[7][0:n], self.buf_q[7][0:n], pen=None, symbol='o', symbolPen=color_line[7])
            self.plt_scatter[5].setSymbolSize(sympole_s)
    def write_reg(self):
    ...
    def update_graph(self):
        ...
        if self.bram4.isChecked():
            ...
            self.plt_scatter[4].clear()
            self.plt_curve_q[4].clear()
            self.plt_curve_i[4].clear()
        ...
        if self.bram7.isChecked():
            ...
            # self.curve_i3.setData(self.fft_i3)
            try:
                self.plt_curve_q[7].setData(self.buf_q[7])
                self.plt_curve_i[7].setData(self.buf_i[7])
                self.plt_curve_q[7].setPos(len(self.buf_q[7]), 0)
                self.plt_curve_i[7].setPos(len(self.buf_i[7]), 0)

                n = min(len(self.buf_q[7]), len(self.buf_i[7]))
                self.plt_scatter[5].setData(self.buf_i[7][0:n], self.buf_q[7][0:n])
            except IndexError:
                self.update_msg('Can NOT find BRAM 7!!!')
                self.bram7.setChecked(0)
            else:
                self.plt_scatter[5].clear()
