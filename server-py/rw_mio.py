...
class MMIO(object):
    ...
    def get_bit_val(self, byte, index):
        ...
        :param index: 0 from right to left 7
        :returns: 0 or 1
        '''
        if byte & (1 << index):
            return 1
    ...
    def set_bit_val(self, byte, index, val):
        '''
        :param byte: origen value
        :param index: the bit to set 0 from right to left 7
        :param val: 0 or 1
        :returns: return changed value
        '''
        if val:
            return byte | (1 << index)
        else:
...
'''
demo = MMIO(0xA0000000, 0x02001ffc)  # defined by FPGA
num_val = 4 # number of value to fetch, each value in format 16 bit (int)
format_str = "h" * num_val # give you "hhhhhhhhhhhhhhhhhhhh...."
txt = demo.read(0x00000000, num_val * 2)  # 0x02000000 is offset defined by FPGA
print(struct.unpack(format_str,txt)) # print data in format int 16 bit
'''
